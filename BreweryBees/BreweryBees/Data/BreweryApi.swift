...
protocol BreweryProtocolAPI {
    ...
    func ratedBrewery(email: String, completion: @escaping (Array<BreweryDefaultModel>) -> ())
}
...
//      MARK: Function to search the brewery
    ...
    func ratedBrewery(email: String, completion: @escaping (Array<BreweryDefaultModel>) -> ()) {
        ...
        guard let sourceURL = URL(string: "\(baseURL)/myEvaluations/\(email.replacingOccurrences(of: "@", with: "%40"))" ) else { return }
        
        URLSession.shared.dataTask(with: sourceURL) { data, _, error in
            
            guard let avaliatedBreweries = data, error == nil else { return }
            
            do {
                let decoder = JSONDecoder()
                let result = try decoder.decode(Array<BreweryDefaultModel>.self, from: avaliatedBreweries)
                print("result \(result)")
                completion(result)
            }
            catch {
